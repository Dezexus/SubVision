...
def apply_lama_inpaint(frame: np.ndarray, roi: Tuple[int, int, int, int], font_size_px: int) -> np.ndarray:
    """Applies LaMa inpainting strictly within the text ROI (green frame)."""
    x, y, w_roi, h_roi = roi
    ...
    mask_dilated = cv2.dilate(mask, dilate_kernel, borderType=cv2.BORDER_CONSTANT, borderValue=0)

    roi_padded, mask_padded, pad_h, pad_w = _pad_to_multiple(roi_slice, mask_dilated, 8)
    ...
    blend_k = min(blend_k, min(bw, bh) | 1)
    soft_mask = cv2.GaussianBlur(mask_dilated, (blend_k, blend_k), 0, borderType=cv2.BORDER_CONSTANT).astype(np.float32) / 255.0
