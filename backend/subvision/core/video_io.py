...
from typing import Iterator, Optional, Tuple, Dict, Any, NamedTuple
...
def get_video_metadata(video_path: str) -> Dict[str, Any]:
    ...
    try:
        with av.open(video_path) as container:
            ...
            return {"width": stream.codec_context.width, "height": stream.codec_context.height, "fps": fps, "total_frames": total_frames}
...
def _open_video_container(video_path: str, use_hwaccel: bool = True) -> Iterator[av.container.InputContainer]:
    ...
    hwaccel_options = {"hwaccel": "cuda", "hwaccel_device": "0"}
    if use_hwaccel and settings.use_nvdec:
        try:
            with av.open(video_path, options=hwaccel_options) as container:
                yield container
                return
        except Exception as e:
            logger.debug("NVDEC open failed, falling back to software decode: %s", e)
